...
_truckersmp_cli() {
    ...
    if [[ "${cur}" == -* ]]; then
        COMPREPLY=( $(compgen -W "-h --help -b --beta -c --configfile --download-throttle -f --flatpak-steam -g --gamedir -i --proton-appid -l --logfile -m --moddir -n --account -o --protondir -p --proton -r --rendering-backend --steamruntimedir -v --verbose -w --wine -x --prefixdir --activate-native-d3dcompiler-47 --check-windows-steam --disable-proton-overlay --disable-steamruntime --game-options --native-steam-dir --self-update --skip-update-proton --use-wined3d --wine-desktop --wine-steam-dir --without-wine-discord-ipc-bridge --json --version" -- "${cur}") )
        return
    fi
    ...
    local acts="start|update|downgrade|updateandstart|ustart|downgradeandstart|dstart|kill|doctor"
    for ((i=1; i < ${COMP_CWORD}; i++)); do
    ...
    COMPREPLY=( $(compgen -W "start update downgrade updateandstart ustart downgradeandstart dstart kill doctor" -- "${cur}") )
} &&
